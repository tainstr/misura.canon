...
class DummyInterface(dict):
	dummy={'Meta_sub':{'value':1,'point':'None','time':9,'temp':10},'fullpath':'/ciao/sub/'}
	devices=[dummy]
	log=logger.Log
	kiln=dummy
	outFile=None
	def __init__(self):
		dict.__init__(self,{'Meta_ciao':{'value':1,'point':'None','time':9,'temp':10},'fullpath':'/ciao/'})
	def child(self,name):
...
class InterfaceEnvironment(unittest.TestCase):
	...
	def test_Metafunc(self):
		self.assertEqual(self.ie.MetaTime('ciao'),self.obj['Meta_ciao']['time'])
		self.assertEqual(self.ie.MetaTemp('ciao'),self.obj['Meta_ciao']['temp'])
		self.assertEqual(self.ie.MetaValue('ciao'),self.obj['Meta_ciao']['value'])
		
	def test_Leaf(self):
		self.assertEqual(self.ie.Leaf('dummy').Opt('Meta_sub'),self.obj.dummy['Meta_sub'])
	...
	def test_eval(self):
		mi=milang.MiLang("L=obj.Leaf('dummy'); t=L.MetaTime('sub'); mi.Value(t)")
		mi.handle='ciao'
		out=DummyInterface()
		ins=DummyInterface()
		mi.eval(out,ins)

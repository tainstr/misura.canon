...
class InterfaceEnvironment(DataEnvironment):
	...
	def obj(self,obj):
		...
		if obj.has_key('fullpath'):
			self.prefix=obj.get('fullpath') # Use get() and not [] because it can be an autoproxy
...
#		print self.obj.keys()
	...
	def Leaf(self,name):
		...
		if r is None:
			raise exceptions.BaseException('Child object does not exist: '+name)
...
class InstrumentEnvironment(InterfaceEnvironment):
	def stop_acquisition(self,save=True):
		# Signal acq stop. We are in different process and cannot directly call stop_acquisition!
		self.obj.log.info('Script is requesting acquisition end. '+self.handle)

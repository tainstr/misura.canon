...
class retry(object):
	...
	def __call__(self, func):
		...
		def retry_loop(*args, **kwargs):
			...
			# it is expected as a property of the first argument (the func's self).
			retry=self.times
			if retry is None:
				retry=args[0].retry
			times=retry
			...
			while True:
				try:
					...
					if self.hook is not False:
						self.hook(func, args, kwargs,  sys.exc_info(), times-retry)
		return retry_loop
...
def stop_profiler(obj):
	out='{}{}_{}.prf'.format(profile_path,obj.__class__.__name__,str(id(obj)))

...
#				print "Found attr",ns,subtree['names']
		...
		if not (okattr and okval):
			self.set_error(node,'Illegal attribute access: %s, %s' % (val,attr))
	...
	def visit_Assign(self,node):
		...
		for val in vals:
			if not self.val_name(val):
				self.set_error(node,'Illegal variable definition to name: %r' % val)
				return False
#		val=node.value
...
#        	self.set_error(node,'Illegal attribute call: %r %r' % (node.func.value,node.func.attr))
	...
	def visit_FunctionDef(self,node):
		...
		if not self.val_name(node.name):
			self.set_error(node,"Invalid function definition name: "+node.name)
		else:
